    ...
    ReentrancyGuard
{
    ...
    modifier validLister() {
        require(
            users[msg.sender].roleType == RoleType.FabricSeller ||
                users[msg.sender].roleType == RoleType.Buyer,
            NotVaildLister()
        );
    ...
    modifier orderExist(uint256 _tokenId, address _buyer) {
        ...
        require(
            order.quantity > 0 && order.orderStatus == OrderStatus.Pending,
            OrderDoesNotExist()
        );
        _;
    }
    ...
    modifier orderNotExists(uint256 _tokenId, address _buyer) {
        Order memory order = orders[_tokenId][_buyer];
        require(
            order.quantity == 0 || order.timeofOrder == 0,
            OrderDoesExist()
        );
        _;
    }
    ...
    ) external validLister validListing(_tokenId, msg.sender) {
    ...
    function updateListing(
        ...
        uint256 _pricePerItem,
        uint256 _quantity
    ) external validLister isListed(_tokenId, msg.sender) {
    ...
    ) external payable validBuyer orderNotExists(_tokenId, msg.sender) {
        require(_quantity > 0, InvalidQuantity());
    ...
    function cancelOrder(
        uint256 _tokenId
    ) external nonReentrant orderExist(_tokenId, msg.sender) {
        // return money
    ...
    function _isOwner(uint _tokenId, address _owner) internal view {
        require(
            IERC721(address(this)).ownerOf(_tokenId) == _owner,
            NotItemOwner()
        );
    }
